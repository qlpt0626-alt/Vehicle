...
export const TaskReportForm: React.FC<Props> = ({ existingFormId, initialData, onSaved, onClose }) => {
  ...
  const [formData, setFormData] = useState(() => {
    return {
      ...
      approverTitle: 'TRUNG ĐỘI TRƯỞNG',
      approverName: 'Thượng úy Huỳnh Quang Long',
      tasks: [...DEFAULT_TASKS]
    };
  });
  ...
  const [docId, setDocId] = useState(() => {
    ...
    const baseId = `TASKREPORT_${Date.now()}`;
    ...
    try {
      ...
      if (foundDoc) {
        if (foundDoc.formData) {
           ...
           if (data.approverTitle === undefined) {
              data.approverTitle = 'TRUNG ĐỘI TRƯỞNG';
           }
           if (data.approverName === undefined) {
              data.approverName = 'Thượng úy Huỳnh Quang Long';
           }
           setFormData(data);
        }
  ...
  const handleDelete = async () => {
    ...
    let confirmed = false;
    try {
      confirmed = window.confirm('Bạn có chắc chắn muốn xóa báo cáo này?');
    } catch (err) {
      confirmed = true;
    }
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden w-full flex sm:justify-center p-2 sm:p-8 print:p-0 print:block" onClick={() => setActiveSelectIndex(null)}>
        ...
        >
          ...
          <div className="text-center mb-8 relative">
            <h1 className="text-xl font-bold uppercase m-0 leading-tight mb-2">BÁO CÁO</h1>
            <h2 className="text-lg font-bold m-0 leading-tight">Kết quả thực hiện nhiệm vụ</h2>
          </div>
