...
const ITEMS: any[] = [
  {
    category: 'I. BỘ CHẾ HÒA KHÍ',
    stt: 1,
    noiDung: "Gic lơ",
    yeuCau: "Sạch sẽ, không tắc bẩn"
  },
  {
    category: 'II. THÙNG NHIÊN LIỆU',
    categoryRequirement: 'Không bị móp méo, cong vênh, biến dạng',
    stt: 1,
  ...
  },
  {
    ...
    categoryRequirement: 'Không bị móp méo, cong vênh, biến dạng',
    stt: 2,
    noiDung: "Ống hút nhiên liệu",
    yeuCau: "Không được sát đáy thùng"
  },
  {
    category: 'III. KÉT LÀM MÁT',
    stt: 1,
  ...
  },
  {
    ...
    stt: 4,
    noiDung: "Van xả đáy",
    yeuCau: "Phải kín và hoạt động tốt"
  }
];
...
export const PartsCleaningRepairForm: React.FC<Props> = ({ vehicle, existingFormId, templateName, stageName, templateType, initialData, onSaved, onClose }) => {
  ...
  const resolvedStageName = stageName || 'Tẩy rửa, làm sạch chi tiết';
  ...
  const loadData = async () => {
    try {
      ...
      const targetType = templateType || 'PARTS_CLEANING_REPAIR';
          ...
          <div className="mb-6 grid grid-cols-2 gap-8 text-[15px] leading-relaxed">
            ...
            <div className="space-y-2">
              <div className="flex gap-2">
                ...
                <input 
                  type="text" 
                  value={formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Tẩy rửa, làm sạch chi tiết'}
                  ...
                  return formData.items.map((item: any, index: number) => {
                    ...
                    return (
                      <React.Fragment key={index}>
                        {showCategoryHeader && (
                          <tr className="bg-stone-50 print:bg-stone-100 font-bold text-blue-800 print:text-black">
                            <td colSpan={item.categoryRequirement ? 2 : 7} className={`border border-black px-4 py-2 text-[14px] uppercase ${item.categoryRequirement ? 'text-center' : 'text-left'}`}>
                              {item.category}
                            </td>
                            {item.categoryRequirement && (
                              <td colSpan={5} className="border border-black px-4 py-2 text-left text-[14px] font-normal italic">
                                {item.categoryRequirement}
                              </td>
                            )}
                          </tr>
