...
export const PaintInspectionBeforeRepairForm: React.FC<Props> = ({ vehicle, existingFormId, templateName, stageName, templateType, initialData, onSaved, onClose }) => {
  ...
  const [formData, setFormData] = useState(() => {
    ...
    return {
      ...
      xxNumber2: '',
      sheetNumber: '',
      items: defaultItems.map(item => ({ ...item, actual: '', evaluation: '', notes: '' })),
  ...
  const [docId, setDocId] = useState(() => {
    ...
    const baseId = vehicle ? `PIR_${vehicle.vehicleId}_${Date.now()}` : `PIR_${Date.now()}`;
  ...
  const loadData = async () => {
    try {
      ...
      const targetType = templateType || 'PAINT_PRE_REPAIR';
  ...
  const handleSave = async () => {
    try {
      ...
      console.log('Đã lưu phiếu kiểm tra sơn.');
